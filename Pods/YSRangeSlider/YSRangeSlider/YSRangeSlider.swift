...
@IBDesignable open class YSRangeSlider: UIControl {
    ...
    private var ablaufBalken:UIView?
    private var ablaufLeftThumb:UIView?
    private var ablaufRightThumb:UIView?
    private var ablaufBalkenWeiteFaktor:CGFloat = 0
    public func resetAblaufTimer(){
        hideAblaufSlider(true)
    }
    public func setAblaufBalkenAndDaumen(_ abgelaufeneZeit:TimeInterval){
        hideAblaufSlider(false)
        
        let gesamtDauer         = TimeInterval(maximumValue)
        
        
        //Daumen eventuell anschalten
        if abgelaufeneZeit > TimeInterval(minimumSelectedValue){
            ablaufLeftThumb?.backgroundColor        = ablaufBalken?.backgroundColor
        }
        if abgelaufeneZeit > TimeInterval(maximumSelectedValue){
            ablaufRightThumb?.backgroundColor       = ablaufBalken?.backgroundColor
        }
        
        if abgelaufeneZeit > gesamtDauer{
            maximumValue = CGFloat(abgelaufeneZeit)
            setNeedsLayout()
            layoutIfNeeded()
        }
        
        let gesamtBreite            = sliderLineLayer.frame.size.width
        ablaufBalkenWeiteFaktor     = CGFloat(abgelaufeneZeit / gesamtDauer) < 1  ? CGFloat(abgelaufeneZeit / gesamtDauer) : 1
        ablaufBalken?.frame.size.width  = gesamtBreite * ablaufBalkenWeiteFaktor
        bringSubview(toFront: ablaufBalken!)
    }
    
    private func hideAblaufSlider(_ isHidden: Bool){
        ablaufBalken?.isHidden              = isHidden
        ablaufLeftThumb?.isHidden           = isHidden
        ablaufRightThumb?.isHidden          = isHidden
        schablonenBalken?.isHidden          = isHidden
        schablonenLeftThumb?.isHidden       = isHidden
        schablonenRightThumb?.isHidden      = isHidden
    }
    
    public func coverSlider(){
        schablonenBalken                            = UIView(frame: sliderLineLayer.frame)
        schablonenBalken?.backgroundColor           = UIColor(white: 0.9, alpha: 1)
        schablonenBalken?.layer.cornerRadius        = sliderLineLayer.cornerRadius
        
        schablonenLeftThumb                         = UIView(frame: leftThumbLayer.frame)
        schablonenLeftThumb?.backgroundColor        = UIColor(white: 0.9, alpha: 1)
        schablonenLeftThumb?.layer.cornerRadius     = leftThumbLayer.cornerRadius
        
        schablonenRightThumb                        = UIView(frame: rightThumbLayer.frame)
        schablonenRightThumb?.backgroundColor       = UIColor(white: 0.9, alpha: 1)
        schablonenRightThumb?.layer.cornerRadius    = rightThumbLayer.cornerRadius
        
        addSubview(schablonenLeftThumb!)
        addSubview(schablonenRightThumb!)
        
        //Ablauf Balken und Daumen
        ablaufBalken                        = UIView(frame: sliderLineLayer.frame)
        ablaufBalken?.backgroundColor       = UIColor(red: 0, green: 128.0/256.0, blue: 0, alpha: 1)
        ablaufBalken?.layer.cornerRadius    = sliderLineLayer.cornerRadius 
        ablaufBalken?.frame.size.height     = schablonenBalken!.frame.height*2/3
        ablaufBalken?.center                = schablonenBalken!.center
        ablaufBalken?.frame.size.width      = 0

        ablaufLeftThumb                     = UIView(frame: leftThumbLayer.frame)
        ablaufLeftThumb?.backgroundColor    = UIColor.white
        ablaufLeftThumb?.layer.cornerRadius = leftThumbLayer.cornerRadius

        ablaufRightThumb                    = UIView(frame: rightThumbLayer.frame)
        ablaufRightThumb?.backgroundColor   = UIColor.white
        ablaufRightThumb?.layer.cornerRadius    = rightThumbLayer.cornerRadius

        addSubview(ablaufLeftThumb!)
        addSubview(ablaufRightThumb!)
        addSubview(schablonenBalken!)
        addSubview(ablaufBalken!)
    
        ablaufBalken?.isHidden              = true
        ablaufLeftThumb?.isHidden           = true
        ablaufRightThumb?.isHidden          = true
        schablonenBalken?.isHidden          = true
        schablonenLeftThumb?.isHidden       = true
        schablonenRightThumb?.isHidden      = true
    }
...
//            updateComponentsPosition()
    ...
    */
    ...
    override open func layoutSubviews() {
        ...
        //Abdeckung
        ablaufBalken?.frame                         = sliderLineLayer.frame
        schablonenBalken?.frame                     = sliderLineLayer.frame
        
        ablaufLeftThumb?.frame      = leftThumbLayer.frame
        ablaufRightThumb?.frame     = rightThumbLayer.frame
        schablonenLeftThumb?.frame  = leftThumbLayer.frame
